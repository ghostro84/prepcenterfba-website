/**
 * js/config.js — Single source of truth for PrepHub Germany.
 * Replace every value marked [PLACEHOLDER] before go-live.
 *
 * Global: PH_CONFIG
 */
(function (global) {
  ...
  const DEFAULT_PRICES = [
    // ── Wareneingang ──────────────────────────────────────────────────────
    { key: "recv_carton",         group: "receiving", label: "Wareneingang Karton (mit Prep)",  unit: "pro Karton",          price: 0.00, note: "Kostenlos bei gebuchter Prep-Leistung",        desc: "Annahme und Einbuchung Ihrer Einlieferungskartons inkl. Stückzahlkontrolle. Bei gleichzeitig gebuchter Prep-Leistung kostenfrei." },
    ...
    { key: "inspection",  group: "prep", label: "Qualitätskontrolle",      unit: "pro Einheit", price: 0.25,                                                desc: "Sichtprüfung auf Beschädigungen, korrekte Artikelanzahl und Unversehrtheit der Verpackung. Prüfbericht mit Fotos im Kundenportal." },
    ...
    // ── Lagerung ──────────────────────────────────────────────────────────
    { key: "storage_free",   group: "storage", label: "Zwischenlagerung (FBA-Prep)", unit: "erste 14 Tage",      price: 0.00, note: "Kostenlos bei gebuchter FBA-Prep-Leistung", desc: "Kostenlose Zwischenlagerung für alle Waren, für die gleichzeitig eine FBA-Prep-Leistung gebucht ist. Maximal 14 Tage." },
    ...
    // ── Gebühren ──────────────────────────────────────────────────────────
    { key: "min_order", group: "fees", label: "Mindestauftragswert", unit: "pro Lieferung", price: 25.00, desc: "Gilt je Einzelauftrag für Prep-Leistungen. Falls der Auftragswert unter 25,00 € liegt, wird die Differenz als Mindestauftragszuschlag berechnet." },
  ];
  ...
  // Keep VOLUME_TIERS as empty array for backward compat with calculator
  const VOLUME_TIERS = [];
  ...
  /**
   * Returns the active discount rate for a given unit count.
   * Returns 0 if below the first tier.
   */
  function getVolumeDiscount(units) {
    var rate = 0;
    VOLUME_TIERS.forEach(function (tier) {
      if (units >= tier.minUnits) rate = tier.discount;
    });
    return rate;
  }

  /**
   * German EUR currency formatting, e.g. 1.234,56 €
   */
  function fmtEUR(n) {
